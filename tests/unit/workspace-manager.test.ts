...
describe('WorkspaceManager', () => {
  ...
  describe('createWorkspaceBlock', () => {
    ...
    it('should attach the created block to the agent', async () => {
      ...
      expect(mockLettaClient.attachMemoryBlock).toHaveBeenCalledWith('agent-456', {
        block_id: 'block-789',
      });
    });
    ...
    it('should throw error when attachment fails', async () => {
      ...
      const mockBlock = { id: 'block-789' };
      mockLettaClient.createMemoryBlock.mockResolvedValue(mockBlock as any);
      mockLettaClient.attachMemoryBlock.mockRejectedValue(new Error('Attachment failed'));

      const consoleSpy = jest.spyOn(console, 'error').mockImplementation(() => {});

      await expect(workspaceManager.createWorkspaceBlock(request)).rejects.toThrow(
        'Attachment failed'
      );
  ...
  describe('updateWorkspace', () => {
    ...
    it('should update workspace status', async () => {
      mockLettaClient.listMemoryBlocks.mockResolvedValue([mockExistingBlock] as any);
      ...
      const result = await workspaceManager.updateWorkspace('agent-456', 'block-789', update);
  ...
  describe('getWorkspace', () => {
    it('should retrieve workspace by block ID', async () => {
      ...
      const result = await workspaceManager.getWorkspace('agent-456', 'block-789');
    ...
    it('should throw error when block not found', async () => {
      ...
      await expect(workspaceManager.getWorkspace('agent-456', 'block-789')).rejects.toThrow(
        'Workspace block block-789 not found'
      );
    });
  });
  ...
  describe('findWorkspaceByTaskId', () => {
    it('should find workspace by task ID', async () => {
      ...
      const result = await workspaceManager.findWorkspaceByTaskId('agent-456', 'task-123');
  ...
  describe('detachWorkspaceBlock', () => {
    it('should detach workspace block from agent', async () => {
      ...
      expect(mockLettaClient.detachMemoryBlock).toHaveBeenCalledWith('agent-456', 'block-789');
    });

    it('should handle detachment failure gracefully', async () => {
      mockLettaClient.detachMemoryBlock.mockRejectedValue(new Error('Detachment failed'));
