import { getTaskFiles, readTaskFile, type GetTaskFilesParams, type ReadTaskFileParams, type FileAccessContext } from "../../src/tools/file-access-tools.js";
  ...
  describe("getTaskFiles", () => {
    ...
    describe("Error handling", () => {
      it("should wrap execution errors with descriptive message", async () => {
        ...
        mockRegistry.getTask.mockReturnValue(mockTask as any);
        mockExecution.isTaskActive.mockReturnValue(true);
        mockExecution.getTaskFiles.mockRejectedValue(
          new Error("OpenCode connection failed")
        );
  ...
  describe("readTaskFile", () => {
    ...
    describe("File reading", () => {
      ...
      it("should pass correct parameters to execution manager", async () => {
        ...
        expect(mockExecution.readTaskFile).toHaveBeenCalledWith(
          "task-123",
          "/dir/subdir/file.txt"
        );
      });
    ...
    describe("Error handling", () => {
      it("should wrap execution errors with descriptive message", async () => {
        ...
        mockRegistry.getTask.mockReturnValue(mockTask as any);
        mockExecution.isTaskActive.mockReturnValue(true);
        mockExecution.readTaskFile.mockRejectedValue(
          new Error("File not found in workspace")
        );
