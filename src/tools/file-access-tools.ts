...
export async function getTaskFiles(
  ...
  if (!execution.isTaskActive(task_id)) {
    throw new Error(`Cannot list files for inactive task. Task status: ${task.status}`);
  }

  try {
    const files = await execution.getTaskFiles(task_id);

    const filteredFiles = path === '/' ? files : files.filter((f) => f.startsWith(path));
...
export async function readTaskFile(
  ...
  if (!execution.isTaskActive(task_id)) {
    throw new Error(`Cannot read files from inactive task. Task status: ${task.status}`);
  }
